...
When(/^User enters validcity into search field$/, () => {
    browser.waitForVisible('#q', 5000)
    	.then(browser.setValue('#q',"CALIFORNIA"));    	    
...
When(/^User make a GET request to "([^"]*)"$/, (url) {
...
Then(/^User get a 200 response$/, () {
    ...
    assert( statusCode == 200 , "Expected a 200 response")
});

Then(/^Response has "([^"]*)"$/, () {
	var body = JSON.parse(context.response.body);
	var Description = "clear sky";
    return assert.equal( body.Weather[1].description , Description, "Invalid");
});